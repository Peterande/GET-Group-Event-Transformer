# ==============================================================================
# Voxelize events into a voxel grid representation.
# ==============================================================================
...
def get_voxel(x, B, sensor_size=(128, 128)):
    y = torch.zeros([len(x), B, sensor_size[0], sensor_size[1]], dtype=x[0].dtype, device=x[0].device)
    for i in range(len(x)):
        y[i] = events_to_voxel_torch(x[i][:, 1], x[i][:, 2], x[i][:, 0], x[i][:, 3], B, sensor_size=sensor_size)
    return y